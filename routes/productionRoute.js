...
PRODUCTION_router.get('/', (req, res) => {
    res.send('this is product route');// this gets executed when user visit http://localhost:3000/production
});

PRODUCTION_router.get('/101', (req, res) => {
    res.send('this is product 101 route');// this gets executed when user visit http://localhost:3000/production/101
});

PRODUCTION_router.get('/102', (req, res) => {
    res.send('this is product 102 route');// this gets executed when user visit http://localhost:3000/production/102
});

PRODUCTION_router.post('/addfood',upload.single('pic_url_file'),addfood);
PRODUCTION_router.post('/order',authenticateJWT,order);
PRODUCTION_router.post('/my/all/orders',authenticateJWT,myAllOrders);
PRODUCTION_router.patch('/edit/product',upload.single('pic_url_file'),authenticateJWT,editFoodItem);
